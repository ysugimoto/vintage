...
func NewCryptoCodec(
	...
	if !ok {
		return nil, errors.FunctionError(name, `Invalid cipher. Valid cipher ident is "aes128", "aes192" or "aes256"`)
	}
	if mode != CBC && mode != CTR {
		return nil, errors.FunctionError(name, `Invalid mode. Valid mode ident is "cbc" or "ctr"`)
	}
	if padding != PKCS7 && padding != NOPAD {
		return nil, errors.FunctionError(name, `Invalid padding. Valid padding ident is "pkcs7" or "nopad"`)
	} else if mode == CTR && padding != NOPAD {
		return nil, errors.FunctionError(name, `When mode is ctr, padding must be "nopad"`)
	}
...
func (c *CryptoCodec) decodeHex(hexKey, hexIv string) (cipher.Block, []byte, error) {
	...
	if err != nil {
		return nil, nil, errors.FunctionError(c.name, "Failed to decode key hex string")
	} else if len(key) != c.size {
		return nil, nil, errors.FunctionError(
			c.name,
	...
	if err != nil {
		return nil, nil, errors.FunctionError(c.name, "Failed to decode iv hex string")
	}

	block, err := aes.NewCipher(key)
	if err != nil {
		return nil, nil, errors.FunctionError(c.name, "Failed to create AES cipher block: %s", err)
	}
